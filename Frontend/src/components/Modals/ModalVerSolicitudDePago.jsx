...
export default function ModalVerSolicitudDePago({
  ...
  // Si no hay data, no renderiza (igual que antes)
  ...
  // Limpieza cuando se abre/cambia solicitud
  useEffect(() => {
    if (!visible) return;
  ...
  const handleAbrirModalOrdenesPago = async () => {
    // Seguridad: solo si está pagada (o ajusta si quieres permitirlo en otros estados)
    if (!isPagada) return;

    setMostrarModalOrdenesPago(true);

    // Si ya las cargaste una vez, no repitas la consulta
      ...
      >
        ...
        >
                        ...
                        <div className="pt-2 border-t border-gray-600 flex justify-between text-sm">
                          ...
                          <span
                            className={`font-medium ${
                              Number(solicitud.monto_total) -
                                Number(solicitud.monto_pagado) ===
                              0
                                ? "text-green-400"
                                : "text-red-400"
                            }`}
          ...
          <div className="mt-4 flex justify-end">
            ...
            {/* ÚNICO BOTÓN QUE MANTENEMOS: Órdenes de pago */}
            {isPagada && (
              <button
          ...
          {/* Modal selector (sin // dentro de JSX) */}
